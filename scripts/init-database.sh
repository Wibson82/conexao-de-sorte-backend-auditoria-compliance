...
echo "🗄️ Inicializando database para conexao-sorte-auditoria-compliance..."
...
echo "🔧 Criando database conexao_audit se não existir..."
mysql -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USERNAME" -p"$DB_PASSWORD" -e "
CREATE DATABASE IF NOT EXISTS conexao_audit
...
echo "✅ Database conexao_audit criado/verificado com sucesso"
