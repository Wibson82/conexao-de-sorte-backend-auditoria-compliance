...
main(){
  log "🔐 Setup secrets (auditoria)"; sudo mkdir -p "$SECRETS_DIR"; sudo chmod 755 "$SECRETS_DIR"
  ...
  DB_NAME=$(az keyvault secret show --vault-name "$VAULT_NAME" --name conexao-de-sorte-auditoria-database-name --query value -o tsv 2>/dev/null || echo "")
  ...
  [[ -z "$DB_NAME" || "$DB_NAME" == "null" ]] && DB_NAME="conexao_sorte_auditoria"
